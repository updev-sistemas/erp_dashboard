...
<div class="card">
    <div class="card-body">
        <div class="row">
            ...
            <div class="col-lg-4 text-right"><a class="btn btn-primary" href="{{ route('env_ctm') }}">Retornar</a></div>
        </div>
        <hr />
        <div class="row">
            <div class="col-lg-12">
                <form action="{{ route('put_enterprise_data',['id' => $enterprise->id]) }}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" value="{{ old('phone',$enterprise->phone) }}"  class="form-control" name="phone" placeholder="Telefone" />
                        @if ($errors->has('phone'))
                        <small class="form-text text-danger">{{ $errors->first('phone') }}</small>
                        @endif
                    </div>
                    <div class="form-group">
                        <label for="fantasia">Estado</label>
                        <input type="text" value="{{ old('state_name',$enterprise->state_name) }}"  class="form-control" name="state_name" placeholder="Nome do Estado" />
                        @if ($errors->has('state_name'))
                        <small class="form-text text-danger">{{ $errors->first('state_name') }}</small>
                        @endif
                    </div>
                    <div class="form-group">
                        <label for="city_name">Cidade</label>
                        <input type="text" value="{{ old('city_name',$enterprise->city_name) }}"  class="form-control" name="city_name" placeholder="Nome da Cidade" />
                        @if ($errors->has('city_name'))
                        <small class="form-text text-danger">{{ $errors->first('city_name') }}</small>
                        @endif
                    </div>
